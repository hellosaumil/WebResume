### [JobPostHighlightsExtension — LLM-Powered Job Relevance Scorer](https://github.com/hellosaumil/JobPostHighlightsExtension)

##### Chrome Extension, LLMs, On-Device AI; JavaScript, Chrome MV3; Gemini, Ollama, Claude

- Built a `Chrome MV3` extension with a `2-stage LLM pipeline` to extract job fields and score resume relevance on a `0-5 scale`.
- Engineered `hybrid refinement` to re-extract missing fields via `JSON schema` constraints after `Stage 1`, improving accuracy on noisy pages.
- Routed `multi-provider AI` across `Gemini Cloud`, `Ollama`, and `on-device Gemini Nano` with pre-warmed `clone()` sessions to cut ~25s init cost.
- Orchestrated `Google Antigravity` and `Claude` agentic workflows for `UI scaffolding` while manually owning the `AI pipeline` and scoring rubric.
...
- Designed an `immersive 3D` solar system app on `Android XR` using `Jetpack Compose` and `SceneCore` spatial APIs with `spatial gesture` controls.
- Developed `planetary mechanics` with `axial` rotation and `orbit` simulation in `Kotlin`, maintaining manual oversight of the core physics engine.
...
- Orchestrated `Google Antigravity` agentic workflows with `Gemini 2.5 Pro` and `Claude Sonnet 4.5` to accelerate boilerplate `spatial UI` generation.
